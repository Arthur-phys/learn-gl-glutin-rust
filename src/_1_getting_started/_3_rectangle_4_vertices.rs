...
// Note there's only four and not 6 like it would be if we were drawing two traingles to make a rectangle
...
pub fn draw_rectangle() {
    ...
    unsafe {
        ...
        gl::BindVertexArray(vao); // since it is bound first, it binds to tthe EBO and VBO (because they are the only ones being bound after it)
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = ControlFlow::Wait;
        ...
        unsafe {
            ...
            // draw two triangles for a rectangle
