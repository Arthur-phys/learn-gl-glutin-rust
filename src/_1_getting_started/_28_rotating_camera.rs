...
use glutin::event::{Event, WindowEvent, DeviceEvent, MouseScrollDelta}; // Event creation for updating view
...
pub fn rotating_camera() {
    ...
    let mut camera_target: Point3<f32> = Point3::new(0.0,0.0,-1.0);
    let up_vector: Vector3<f32> = Vector3::new(0.0,1.0,0.0);
    let mut camera_pos: Point3<f32> = Point3::new(0.0,0.0,3.0);
    let mut camera_speed = 0.5;

    // Camera rotating (Euler angles)
    let mut yaw: f32 = -90.0_f32; // start looking towards the negative z-axis (as is defined by the camera view)
    let mut pitch: f32 = 0.0_f32;
    let mut camera_direction = Vector3::new(yaw.to_radians().cos()*pitch.to_radians().cos(),
    ...
    let mut proj_mat: Matrix4<f32> = cgmath::perspective(Deg(fov), 800.0/600.0, 0.1, 100.0);
    let model_mat: Matrix4<f32> = Matrix4::from_axis_angle(Vector3::new(0.5,1.0,0.0), Deg(0.0));
    ...
    // Make cursor dissapear and be captured
    new_context.window().set_cursor_grab(true).unwrap();
    new_context.window().set_cursor_visible(false);
    ...
    event_loop.run(move |event, _, control_flow| {
     ...
     // important to keep speed equally allong simulations/games

     let current_time = time.elapsed().as_millis();
     delta_time = (current_time - last_frame) as f32;
     ...
     // #############################
         ...
         match event { // subscribing to events occurs here
              ...
              Event::WindowEvent {event, ..} => match event {
                   ...
                   WindowEvent::KeyboardInput {input, is_synthetic, ..} => {
                         ...
                         // UP
                         } else if input.scancode == 103 {

                              camera_pos = Point3::new(camera_speed*camera_target.x + camera_pos.x,
                                   camera_speed*camera_target.y + camera_pos.y, camera_speed*camera_target.z + camera_pos.z);
                         // DOWN
                         } else if input.scancode == 108 {

                              camera_pos = Point3::new(-camera_speed*camera_target.x + camera_pos.x,
                                   -camera_speed*camera_target.y + camera_pos.y, -camera_speed*camera_target.z + camera_pos.z);
                         // RIGHT
                         } else if input.scancode == 106 {

                              let camera_target_vec = Vector3::new(camera_target.x,camera_target.y,camera_target.z);
                              let camera_target_right = camera_target_vec.cross(up_vector.clone()).normalize();
                              camera_pos = Point3::new(camera_speed*camera_target_right.x + camera_pos.x,
                                   camera_speed*camera_target_right.y + camera_pos.y, camera_speed*camera_target_right.z + camera_pos.z);
                         // LEFT
                         } else if input.scancode == 105 {
                              let camera_target_vec = Vector3::new(camera_target.x,camera_target.y,camera_target.z);
                              let camera_target_right = camera_target_vec.cross(up_vector.clone()).normalize();
                              camera_pos = Point3::new(-camera_speed*camera_target_right.x + camera_pos.x,
                                   -camera_speed*camera_target_right.y + camera_pos.y, -camera_speed*camera_target_right.z + camera_pos.z);
              ...
              Event::DeviceEvent { device_id, event } => {
               match event {
                    DeviceEvent::MouseMotion { delta: (x,y) } => {
                         let xoffset = (x as f32) * sensitivity;
                         let yoffset = (-y as f32) * sensitivity;
                         yaw += xoffset;
                         pitch += yoffset;
                         // maximum and minimum value for pitch
                         if pitch < -89.0 {
                              pitch = -89.0;
                         } else if pitch > 89.0 {
                              pitch = 89.0;
                         }
                         camera_direction = Vector3::new(yaw.to_radians().cos()*pitch.to_radians().cos(),
                              pitch.to_radians().sin(),yaw.to_radians().sin()*pitch.to_radians().cos());
                         camera_target = Point3::new(camera_direction.x,camera_direction.y,camera_direction.z);

                    },
                    DeviceEvent::MouseWheel { delta } => {
                         if let MouseScrollDelta::LineDelta(x,y) = delta {
                              fov -= y*sensitivity*0.1;
                              if fov < 1.0 {
                                   fov = 1.0;
                              } else if fov > 45.0 {
                                   fov = 45.0;
                              }
                              proj_mat = cgmath::perspective(Deg(fov), 800.0/600.0, 0.1, 100.0);
                              shader_program.set_mat4("projection", &proj_mat);
                         }
                    }
                    _ => ()
         ...
         unsafe {
              ...
              let point_center = Point3::new(camera_target.x + camera_pos.x, camera_target.y + camera_pos.y, camera_target.z + camera_pos.z);
              let view_mat = Matrix4::look_at_rh(camera_pos, camera_target, up_vector);
