...
pub fn camera_roam_box() {
    ...
    let z_vector: Vector3<f32> = Vector3::new(0.0,1.0,0.0);
    ...
    event_loop.run(move |event, _, control_flow| {
         ...
         unsafe {
              ...
              let x_coord = time.elapsed().as_secs_f32().sin()*10.0;
              let z_coord = time.elapsed().as_secs_f32().cos()*10.0;
              let camera_pos = Point3::new(x_coord,0.0,z_coord);
              let view_mat = Matrix4::look_at_rh(camera_pos, camera_target, z_vector);
